module.exports = {
    ...
    updateEvent: async (req, res) => {
        ...
        const {title, description, starting_day, ending_day, starting_time, ending_time, phone_number} = req.body
        const {userId} = req.session.user
        const userEvents = await db.update_user_event({userId, title, description, starting_day, ending_day, starting_time, ending_time, phone_number})
        return res.status(200).send(userEvents)
    },
    deleteEvent: async (req, res) => {
        const db = req.app.get('db')
        const {event_id} = req.params
        db.delete_event([event_id]).then(result => {
